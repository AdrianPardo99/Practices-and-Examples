...
#include "Queue.h"
void Leer(FILE*);
void GLC(Cola,Cola2,Cola);
void Cadenas(Cola,Cola2,Cola,Elem);
void Tran(Cola,Cola2,Cola,Elem,Elem,Elem,Elem,Elem,Elem,Cola);

/*Función que permite la lectura del archivo txt para crear el GLC a partir de una
...
void Leer(FILE *f){
	...
	/*Creacion de las transiciones que necesita cada conjunto de estados del GLC*/
	for(i=0;i!=num;i++){
		centinela=primero(b);
...
/*Función que imprime en un formato bonito (en Linux) el GLC para despues trabajar con el*/
void GLC(Cola a,Cola2 b,Cola c){
	int i,j=0;
	Elem alf,cen;
	printf("GLC={VN,VT,S,F,δ}\n");
	impCola(a,0);
...
/*Función que permite la lectura de una cadena de texto para trabajar en el GLC*/
void Cadenas(Cola a,Cola2 b,Cola c,Elem e){
	Elem cad,efinal2;
	int ban1=0,i,j;
	cad=(Elem)malloc(sizeof(char)*1000);
	printf("Ingresa una cadena para el GLC:\n");
	...
	for(i=0;i!=tamElem(cad);i++){
		if(ban1==1){
			printf("Los caracteres que ingreso no son validos en el alfabeto del GLC\n");
			free(cad);
			exit(0);
			break;
		}
	...
	if(ban1==0){
		...
		Tran(a,b,c,e,cad,estado,caractes,siguientes,efinal,f);
	}else{
		...
		for(i=0;i!=tamElem(efinal);i++){
			if(*(efinal+i)==','){
				*(efinal2+j+1)='\0';
				printf("Estado final: %s\n", efinal2);
				Tran(a,b,c,e,cad,estado,caractes,siguientes,efinal2,f);
				*efinal2='\0';
				j=0;
			}else{
				*(efinal2+j)=*(efinal+i);
...
/*Función que realiza las transiciones de forma recursiva de la cadena en el GLC*/
void Tran(Cola a,Cola2 b,Cola c,Elem e,Elem cad,Elem estado,Elem caracteres,Elem siguientes,Elem efinal,Cola f){
	...
	*(a1+j+1)='\0';
	...
	for(i=0;i!=tamElem(caracteres);i++){
		sigui=primero(d);
		if(*(caracteres+i)==*(cad)){
			Elem cadeni=malloc(sizeof(char)*100),cadeni2=malloc(sizeof(char)*100);
			*cadeni2=*cad;
			strcat(cadeni,cadeni2);
			strcat(cadeni,primero(d));
			f=formar(f,cadeni);
			printf("%s:==%c%s,",estado,*cad,primero(d));
			/*Llamo recursivamente a la funcion para seguir con los demas caminos del GLC*/
			Tran(a,Armar(cen,tamCola(a)-4),c,e,cad+1,sigui,caracteres,siguientes,efinal,f);
		}else{
		}
		d=desformar(d);
	}
